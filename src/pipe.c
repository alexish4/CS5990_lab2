...
static void ic_touch(InstructionCacheSet *set, int way) {
    icache.use_clock++;
    set->ways[way].last_used = icache.use_clock;
}

int icache_hit(uint32_t pc) {
    uint32_t idx = ic_index(pc);
...
int icache_advance_miss(void) {
    if (icache.miss_stall > 0) {
        icache.miss_stall--;
        return icache.miss_stall == 0;
    }
...
void icache_fill_current_miss(void) {
    uint32_t idx = ic_index(icache.miss_block_pc);
    uint32_t tag = ic_tag(icache.miss_block_pc);
    InstructionCacheSet *set = &icache.sets[idx];
...
void pipe_stage_fetch()
{
    ...
    if (icache.miss_stall > 0) {
        if (icache_advance_miss()) {
            icache_fill_current_miss();
        } else {
            return;
        }
    }

    if(!icache_hit(pipe.PC)) {
        icache_start_miss(pipe.PC);
        return;
    }
